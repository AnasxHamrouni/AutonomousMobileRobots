...
- Your robot has a camera already publishing images (e.g., on `/camera/image_raw`).
- The robot can be driven by publishing `geometry_msgs/Twist` messages on `/cmd_vel`.
...
- Subscribes to the camera topic (`/camera/image_raw`).
...
class RedCircleFollower(Node):
    def __init__(self):
        ...
        self.declare_parameter('area_threshold_percentage', 30.0)
...
setup(
    ...
    entry_points={
        'console_scripts': [
            'red_circle_follower = red_circle_follower.red_circle_follower:main'
        ],
    },
)
```
...
- Check the `cmd_vel` topic:
    ...
    ```
- checkthe publishing freq:
    ```bash
    ros2 topic hz /camera_sensor/image_raw
    ```
